---
title: About
permalink: about
comment: ''
blocks:
  - type: callout_block
    lead: Acid Fairies is an independent brand made by 3 friends who create for those who want to embrace their unique style!!
    body: At Acid Fairies, fashion isn’t just about clothes—it’s a way to express your individuality, your personality and everything that makes you, you. We want to create a space where everyone is  comfortable in what they choose to wear. Our mission is to empower people to express who they truly are without stereotypes or fear of judgment.
    button: shop
    link: /shop
    name: Meet our team
    image: /content/media/pages/650776de4e4d20ae25ff32b5037e032e.jpg
  - type: callout_block
    size: h1
    lead: Our team designer and seamstress
    body: dupa dupa dupa
    button: ''
    link: ''
    name: Mania
    image: /content/media/pages/MANIA SOLO ten sam filtr.jpeg
  - type: callout_block
    size: h1
    lead: The lead designer
    body: ania wymysl cos
    button: ''
    link: ''
    name: Ania
    image: /content/media/pages/9adf0807693920110fb352b662bedb82.jpg
  - type: callout_block
    size: h1
    lead: Our teams amazing photographer
    body: magda twoja kolej
    button: ''
    link: ''
    name: Magda
    image: /content/media/pages/MAGDA SOLO.jpeg
  - type: callout_block
    size: h1
    lead: As a small brand, we focus on making the pieces responsibly, in small batches and with attention to the environmental impact.
    body: |-
      All our items are handmade in Poland. We prioritise quality, fair prizing and ethical production. 

      Our collections aren’t rushed. Months of refining go into every design - adjusting the fit, perfecting the details, and making sure it’s something you’ll actually want to wear over and over again.

      Unlike mass production, our approach is community-driven, focusing on creating pieces that resonate for the long term.

      There is always room for improvement, and we are constantly exploring new ways to make responsible choices while ensuring that our garments remain accessible to as many people as possible, so our community can continue to support and enjoy what we create.

      We don’t mass produce, and we don’t sit on piles of unsold stock. Every collection is made in limited quantities, keeping things intentional and reducing waste from the start.

      Our goal is to create fashion that lasts, not trends that fade.
    button: shop
    link: /shop
    name: Sustainability
    image: /content/media/pages/IMG_6898.JPG
---
